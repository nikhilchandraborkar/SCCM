...
#Revision 1: Added AD validation for machine being imported to the tool (suggested by Arun)
#Revision 2: Added a new Tab “Software deployment” to deploy the software ( Suggested by Philip)
################################################################################################################################################
...
[string]$Container1 = '$operating system deployment' #Folder containing OSD collections.
[string]$Container = '$App Deployment Collections'
#[Parameter(Mandatory=$true)]
...
Function Set-RadioButton {
    ...
    ElseIf ($RadioButton3.Checked -eq $True) {
        $ImportButton.Text = "ADD Machine to the collection"
        $AddToSelectionButton.Visible = $True
        $BrowseButton.Enabled = $False
        $BrowseButton.Visible = $False
        $RemoveFromSelectionButton.Enabled = $True
                ...
                If ($Devices -eq $Null) {
                    ...
                    Foreach ($Device in $Devices) {
                        ...
                        ElseIf (($Devices -ne $Null) -and ($MACAddress -eq $Devices.MACAddresses)) {
                            $Device = $($Device.Netbiosname)
                            Add-DirectMembership -DeviceName $Device -CollectionName $($CollectionName)
                            Write-StatusBox -Type INFO -StatusBoxMessage "Adding $($Device) to $($CollectionName) for software installation"
                            If (($LogPath.Length -gt 1) -or ($Script:LogPathExists -eq $True)) {
                                Write-Log -Severity 1 -Component "Reinstall Workstation" -Message "Adding $($Device) to $($CollectionName) for software installation using account: $($Env:Username)."  
                            } 
                        }
                    }  
                }  
...
#Software Deployment Label

$softwaredeploymnet = New-Object System.Windows.Forms.Label
$softwaredeploymnet.AutoSize = $True
$softwaredeploymnet.Font = New-Object System.Drawing.Font("Tahoma",8.25,0,3,0)
$softwaredeploymnet.Location = "375,15"
$softwaredeploymnet.Name = "SoftwareDeployment"
$softwaredeploymnet.Text = "Software Deployment"
$Form.Controls.Add($softwaredeploymnet)
...
#Software Deployment Button
$RadioButton3 = New-Object System.Windows.Forms.RadioButton
$RadioButton3.AutoSize = $True
$RadioButton3.Checked = $False
$RadioButton3.Location = "350, 15"
$RadioButton3.Add_CheckedChanged({Set-RadioButton -RadioButton RadioButton3 -State Enabled})
